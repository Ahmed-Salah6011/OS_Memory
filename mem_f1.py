...
def draw_mem( x, y,z):
    ...
    for i in range(len(x)):
        x[i] = int(x[i])
        y[i] = int(y[i])
        canvas.create_rectangle(100, (((x[i]/z)*600)+50), 300, ((((x[i]+y[i])/z)*600)+50), width=5,outline='#ebebeb', fill='#ebebeb')
...
def draw_segment(base, size, name,z):
    rect=canvas.create_rectangle(100, ((base/z)*600)+50, 300, ((base+size/z)*600)+50, width=5,outline='#ffe873', fill='#ffe873')
    t1=canvas.create_text(80,((base/z)*600)+50, text=str(base))
    t2=canvas.create_text(80, ((base+size/z)*600)+50, text=base+size)
    t3=canvas.create_text(180, (((base+size/z)*600)+50+((base/z)*600) ) / 2, text=name)
            ...
            if (sum(self.H_size) >= sum(segments_size)):
                ...
                if (len(indeces) == len(segments_size)):
                    ...
                    for index in indeces:
                        ...
                        draw_segment(self.H_base[index], segments_size[i], 'P{}-'.format(self.p) + segments_name[i],self.Mem_size)
